...
def metrics_with_ocr_recovery(detail_recovered: pd.DataFrame) -> pd.DataFrame:
    """Calcula métricas por modelo usando el detail donde las no_encontrada
    recuperadas por ocr_corregido ya están marcadas como 'recuperada_ocr'.
    (Aplica sobre el TOTAL de las entidades principales).
    """
    if detail_recovered.empty:
        return pd.DataFrame()
        
    rows = []
    for model, group in detail_recovered.groupby("modelo"):
        gold_mask = group["gold_id"].astype(str).ne("")
        pred_mask = group["pred_id"].astype(str).ne("")
        ...
        recuperadas_mask = gold_mask & (group["tipo_resultado"] == "recuperada_ocr")
        recuperadas = int(recuperadas_mask.sum())
        
        correct_mask = gold_mask & pred_mask
        relaxed_correct = int((correct_mask & group["tipo_resultado"].isin(RELAXED_CORRECT)).sum())
        
        no_encontrada_post = int((gold_mask & (group["tipo_resultado"] == "no_encontrada")).sum())
        no_encontrada_orig = no_encontrada_post + recuperadas
        
        precision = _safe_div(relaxed_correct, pred_total)
        recall = _safe_div(relaxed_correct + recuperadas, gold_total)
        ...
        rows.append({
            ...
            "total_entidades_gold": gold_total,
            "total_entidades_predichas": pred_total,
            "recuperadas_ocr": recuperadas,
            "no_encontrada_original": no_encontrada_orig,
            "no_encontrada_post_recuperacion": no_encontrada_post,
            "precision_con_recuperacion": precision,
            "recall_con_recuperacion": recall,
            "f1_con_recuperacion": f1,
        })
        
    return pd.DataFrame(rows)

def metrics_with_ocr_recovery_isolated(detail_recovered: pd.DataFrame, recovery_labels: set[str]) -> pd.DataFrame:
    """Calcula métricas por modelo de forma aislada, tomando en cuenta ÚNICAMENTE
    las etiquetas configuradas para recuperación (ej: persona).
    """
    if detail_recovered.empty or not recovery_labels:
        return pd.DataFrame()
        
    rows = []
    for model, group in detail_recovered.groupby("modelo"):
        # Solo las etiquetas de recuperación y alcance principal
        gold_mask = group["gold_id"].astype(str).ne("") & group["etiqueta_gold"].isin(recovery_labels) & group["gold_incluida_principal"].astype(bool)
        pred_mask = group["pred_id"].astype(str).ne("") & group["etiqueta_predicha"].isin(recovery_labels) & group["pred_incluida_principal"].astype(bool)
